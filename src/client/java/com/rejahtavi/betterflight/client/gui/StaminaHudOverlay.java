...
import static com.linguardium.betterflight.util.Utilities.id;
...
public class StaminaHudOverlay implements HudRenderCallback
{
    ...
    private static final Identifier staminaIcons =
            id( "textures/elytraspritesheet.png");
...
//    }
    ...
    public static void registerOverlay() {
        HudRenderCallback.EVENT.register(new ClassicHudOverlay());
    }
    @Override
    public void onHudRender(DrawContext drawContext, float tickDelta) {
        MinecraftClient client = MinecraftClient.getInstance();
        renderOverlay(client.currentScreen, drawContext,tickDelta, client.getWindow().getWidth(), client.getWindow().getHeight());


    }
    public static void renderOverlay(@Nullable Screen gui, DrawContext context, float tickDelta, int screenWidth, int screenHeight)
    {
        if (!ClientData.isFlightEnabled() || !ClientData.isWearingFunctionalWings()) return;
        //if(ClientConfig.CLIENT.classicHud.get()) return;

        MinecraftClient client = MinecraftClient.getInstance();
        if (client.options.hudHidden || !(client.interactionManager != null && client.interactionManager.hasStatusBars())) return;
        ...
        boolean shouldAnimate = (client.world != null && !client.isPaused());
        if (durability == LOW_DURABILITY)
        {
            if (shouldAnimate) {
                long thisTick = client.world.getTime();
        ...
        y = y /*- rightOffset*/ + shakeY;
        //Render Empty Resources
        for (int i = 0; i < 10; i++)
        {
            context.drawTexture(staminaIcons, getXPos(x, i) + shakeX, y,
                    NONE, durability,
